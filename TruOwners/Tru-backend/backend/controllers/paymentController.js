...
const initiatePayment = async (req, res) => {
  ...
  try {
    ...
    if (!plan) {
      return res.status(404).json({
        ...
        error: {
          ...
          details: `No subscription plan found with ID: ${planId}`,
        },
      });
    }
    ...
    const merchantTransactionId = `TRU_${Date.now()}_${userId
      .toString()
      .slice(-6)}`;
  ...
  try {
    ...
    if (!merchantOrderId) {
      ...
      return res.status(400).json({
        success: false,
        error: {
          message: "Missing merchant order ID",
          details:
            "The PhonePe callback did not contain a valid merchantTransactionId or merchantOrderId.",
        },
      });
    }
    ...
    if (!payment) {
      ...
      return res.status(404).json({
        ...
        error: {
          ...
          details: `No payment record found matching merchant order ID: ${merchantOrderId}`,
        },
      });
    }
  ...
  try {
    ...
    if (!payment) {
      return res.status(404).json({
        ...
        error: {
          message: "Payment not found",
          details: `No payment record exists for merchantTransactionId: ${merchantTransactionId}`,
        },
      });
    }
